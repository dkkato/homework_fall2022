    ...
    def update(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        """
            ...
            returns:
                training loss
        """
        # TOD: Implement the pseudocode below: do the following (
        ...
        for k in range(self.num_target_updates):
          v_s_p = self.critic_network(next_ob_no).squeeze()
          targets = reward_n + self.gamma*v_s_p*(1-terminal_n)
          targets = targets.detach()
          for j in range(self.num_grad_steps_per_target_update):
            inputs = self.critic_network(ob_no).squeeze()
